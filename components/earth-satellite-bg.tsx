...
export function BinaryRainBackground() {
  ...
  useEffect(() => {
    ...
    const drawSatellite = (ctx: CanvasRenderingContext2D, width: number, height: number, time: number) => {
      // Fixed Earth on the right side
      const earthX = width * 0.75 // Fixed position on the right
      const earthY = height * 0.6
      const earthRadius = Math.min(width, height) * 0.3
      
      // Satellite orbiting Earth realistically
      const orbitRadius = earthRadius * 1.2
      const orbitSpeed = 0.8 // Slower, more realistic orbit
      const satelliteAngle = time * orbitSpeed
      
      // 3D elliptical orbit for depth effect
      const satelliteX = earthX + Math.cos(satelliteAngle) * orbitRadius * 0.9
      const satelliteY = earthY + Math.sin(satelliteAngle) * orbitRadius * 0.6
      
      // Calculate depth (z-axis) for 3D effect
      // When satelliteAngle is between π/2 and 3π/2, it's "behind" Earth
      const depth = Math.sin(satelliteAngle) // -1 (behind) to 1 (front)
      const isBehind = depth < 0
      
      // Calculate opacity based on depth (more transparent when behind)
      const opacity = isBehind ? 0.4 : 1.0 // Semi-transparent when behind Earth
      const scale = isBehind ? 0.8 : 1.0 // Slightly smaller when behind
      
      // Satellite rotation (self-rotation, not orbit rotation)
      const satelliteRotation = time * 2

      // Draw Earth first (so satellite behind it gets hidden)
      ctx.save()
      ...
      // Draw satellite with depth effect (always visible but transparent when behind)
      ctx.save()
      ctx.translate(satelliteX, satelliteY)
      ctx.rotate(satelliteRotation * 0.1)
      ...
      // Satellite body
      ctx.fillStyle = "#00ffff" // Cyan instead of green to stand out from binary
      ctx.shadowColor = isBehind ? "rgba(0, 255, 255, 0.3)" : "#00ffff"
      ctx.shadowBlur = isBehind ? 5 : 15
      ctx.beginPath()
      ...
      // Solar panels
      ctx.fillStyle = isBehind ? "rgba(0, 128, 255, 0.6)" : "#0080ff"
      ctx.fillRect(-25, -4, 12, 8)
      ctx.fillRect(13, -4, 12, 8)

      // Satellite details
      ctx.fillStyle = isBehind ? "rgba(255, 255, 255, 0.6)" : "#ffffff"
      ctx.fillRect(-3, -2, 6, 4)

      ctx.restore()

      // Communication waves (only visible when in front)
      if (!isBehind) {
        ctx.save()
    ...
    const drawStarsAndParticles = (ctx: CanvasRenderingContext2D, particles: any[], width: number, height: number, time: number) => {
      const scrollX = (time * 30) % (width + 200) // Slower scroll for parallax effect
      
      particles.forEach((particle, index) => {
        // Update position with slow scroll
        particle.x += particle.vx
        particle.y += particle.vy
        
        // Apply scroll effect (parallax)
        const scrollOffset = scrollX * 0.5 // Slower than background elements
    ...
    const drawBinaryRain = (ctx: CanvasRenderingContext2D, drops: any[], width: number, height: number, time: number) => {
      const scrollX = (time * 20) % (width + 200) // Very slow scroll for binary rain
      
      drops.forEach((drop, index) => {
        // Update position
        drop.y += drop.speed
        drop.x += Math.sin(time + index * 0.1) * 0.1 // Very slight horizontal drift
